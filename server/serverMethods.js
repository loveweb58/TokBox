...
function ServerMethods(aLogLevel, aModules) {
  ...
  function getRoomInfo(aReq, aRes) {
    ...
    var fbArchives = tbConfig.fbArchives;
    var roomName = aReq.params.roomName.toLowerCase();
    var usrnameString = aReq.query.userName.split('?');
    logger.log('in server');
    logger.log('query' + JSON.stringify(aReq.query));
    logger.log('request' + JSON.stringify(aReq.params));
    var userName = usrnameString[0];
    var userId = usrnameString[1].split('=');
    userId = userId[1];
    logger.log('getRoomInfo serving ' + aReq.path, 'roomName: ', roomName, 'userName: ', userName, 'userid: ', userId);
        ...
        var answer = {
          ...
          token: tbConfig.otInstance
            .generateToken(usableSessionInfo.sessionId, {
              role: 'publisher',
              data: JSON.stringify({ userName, userId, roomName }),
            }),
          username: userName,
          firebaseURL:
