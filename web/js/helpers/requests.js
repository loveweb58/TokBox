!function (exports) {
  ...
  function getRoomInfo(aRoomParams) {
    console.log("in room info");
    console.log(aRoomParams);
    var userName = aRoomParams.username ? '?userName=' + aRoomParams.username : '';
    var userId = aRoomParams.userId ? '?userId=' + aRoomParams.userId : '';

    return sendXHR('GET', server + '/room/' + aRoomParams.roomURI + '/info' + userName + userId).
