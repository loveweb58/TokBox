...
!(function (exports) {
  ...
  var viewEventHandlers = {
    endCall: function () {
      ...
      // window.location=baseUrl+"/video/getSessionInfo/?id=summary";
      otHelper.disconnect();
  ...
  var _allHandlers = {
    ...
    streamCreated: function (evt) {
      publisherReady.then(function () {
        console.log("new stream is jere from roomController");
        console.log("no of users now" + numUsrsInRoom);
  ...
  function getRoomParams() {
    ...
    //change room name from url
    var roomNameFromUrl = params.getFirstValue('roomName');
    var userIdFromUrl = params.getFirstValue('userId');
  ...
  function getRoomInfo(aRoomParams) {
    return Request
      ...
      .then(function (aRoomInfo) {
        ...
        aRoomInfo.userId = aRoomParams.userId;
  ...
  var init = function () {
    LazyLoader.load(modules)
      ...
      .then(function (aParams) {
        ...
        console.log('from cont' + JSON.stringify(aParams));
